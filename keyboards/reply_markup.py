...
from db.models import MediaCategory, User, District, Tournament
from localization.strings import get_all_languages
...
async def main_menu_tm(user: User) -> dict:
    ...
    markup = ReplyKeyboardMarkup(
        ...
        keyboard=[
            ...
            [
                KeyboardButton(text=_("tournaments", user.lang_code)),
            ],
            [
...
def translated_button(user: User, key: str):
...
async def get_tournaments_list(user: User) -> dict | None:
    ...
    active_tournaments = await Tournament.filter(
        Q(active=True) & Q(start_date__lte=current_date) & Q(end_date__gte=current_date)
    )

    for tournament in active_tournaments:
        markup.button(text=tournament.name)
    markup.row(translated_button(user, 'cancel').keyboard[0][0])
    if len(active_tournaments) == 0:
        return None
    
    return {"text": text, "reply_markup": markup.as_markup(resize_keyboard=True)}
