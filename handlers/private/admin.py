...
from filters.common import IsAdmin, CommentReply
...
async def ask_broadcast_message(message: types.Message, state: FSMContext):
    await message.answer(text='Send your message to be broadcast:', reply_markup=cancel_button)
...
@router.message(AdminState.input_broadcast_message,
                F.content_type.in_({'text', 'audio', 'photo', 'video', 'video_note', 'location', 'document'}))
async def start_broadcasting(message: types.Message, user: User, state: FSMContext):
    ...
    percent_range = int(users_count * .01) * 5
